...
class RepaymentsReallocationService @Inject() (
  repaymentsReallocationsConnector: RepaymentsReallocationsConnector
) extends Logging {

  def getRepayReallocationSummary(taxRef: Long, accPeriod: Long)(implicit
    hc: HeaderCarrier
  ): Future[RepayReallocationSummary] =
    repaymentsReallocationsConnector.getRepayReallocationSummary(taxRef, accPeriod)
